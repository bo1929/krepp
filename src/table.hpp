...
public:
  ...
  FlatHT(tree_sptr_t tree, crecord_sptr_t crecord)
    : tree(tree)
    , crecord(crecord) {};
  tree_sptr_t get_tree();
  inc_t get_inc(uint32_t rix);
  crecord_sptr_t get_crecord();
  void set_tree(tree_sptr_t source);
  void set_crecord(crecord_sptr_t source);
  std::vector<cmer_t>::const_iterator begin();
  std::vector<cmer_t>::const_iterator end();

...
void DynTable::union_row(vec<mer_t> &dest_v, vec<mer_t> &source_v,
  ...
  if (in_place) {
    ...
    std::inplace_merge(dest_v.begin(),
                       dest_v.begin() + dest_v.size() - source_v.size(),
  ...
  assert(iter != dest_v.end());
  while (++iter != dest_v.end()) {
    if (!(result->encoding == iter->encoding) && ++result != iter)
...
void DynTable::fill_table(reader_sptr_t reader) {
  while (reader->read_next_seq() && reader->make_ready()) {
    reader->extract_mers(table);
  }
  sort_columns();
  make_unique();
}

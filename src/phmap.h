...
// ---------------------------------------------------------------------------
    ...
    #pragma warning(                                                                               \
      disable : 4868) // compiler may not enforce left-to-right evaluation order in braced initializer list
...
namespace phmap {
  ...
  namespace priv {
    ...
    {
      alignas(16) static constexpr ctrl_t empty_group[] = {kSentinel,
                                                           kEmpty,
                                                           kEmpty,
                                                           kEmpty,
                                                           kEmpty,
    ...
    public:
      ...
      using pointer =
        typename phmap::allocator_traits<allocator_type>::template rebind_traits<value_type>::pointer;
    ...
    private:
      ...
      using RequiresInsertable = typename std::enable_if<
        phmap::disjunction<std::is_convertible<T, init_type>, SameAsElementReference<T>>::value,
        int>::type;
    ...
    public:
      ...
      public:
        ...
        using reference = phmap::
          conditional_t<PolicyTraits::constant_iterators::value, const value_type&, value_type&>;
      ...
      raw_hash_set(raw_hash_set&& that) noexcept(
        std::is_nothrow_copy_constructible<hasher>::value&& std::is_nothrow_copy_constructible<
          key_equal>::value&& std::is_nothrow_copy_constructible<allocator_type>::value)
      ...
      PHMAP_ATTRIBUTE_REINITIALIZES void clear()
      {
        ...
        if (capacity_) {
          PHMAP_IF_CONSTEXPR(
            (!std::is_trivially_destructible<typename PolicyTraits::value_type>::value ||
         ...
         IsNoThrowSwappable<allocator_type>(typename AllocTraits::propagate_on_container_swap{})))
      {
        ...
        SwapAlloc(
          alloc_ref(), that.alloc_ref(), typename AllocTraits::propagate_on_container_swap{});
      }
    ...
    private:
      ...
      void initialize_slots(size_t new_capacity)
      {
        ...
        char* mem =
          static_cast<char*>(Allocate<Layout::Alignment()>(&alloc_ref(), layout.AllocSize()));
      ...
      void resize(size_t new_capacity)
      {
        ...
        for (size_t i = 0; i != old_capacity; ++i) {
          if (IsFull(old_ctrl[i])) {
            size_t hashval =
              PolicyTraits::apply(HashElement{hash_ref()}, PolicyTraits::element(old_slots + i));
            auto target = find_first_non_full(hashval);
    ...
    private:
      ...
      std::tuple<size_t /* growth_left */, hasher, key_equal, allocator_type> settings_{
        0,
        hasher{},
        key_equal{},
        allocator_type{}};
    ...
    class raw_hash_map : public raw_hash_set<Policy, Hash, Eq, Alloc>
    {
      ...
      using MappedReference =
        decltype(P::value(std::addressof(std::declval<typename raw_hash_map::reference>())));
    ...
    (defined(_MSC_VER) || defined(__clang__) || (defined(__GNUC__) && __GNUC__ > 6))
      ...
      parallel_hash_set(const parallel_hash_set& that)
        : parallel_hash_set(that,
                            AllocTraits::select_on_container_copy_construction(that.alloc_ref()))
      {}
      ...
      {
        return const_cast<parallel_hash_set*>(this)->template modify_if_impl<K, F, SharedLock>(
          key, std::forward<F>(f));
      }
      ...
      {
        std::for_each(
          std::forward<ExecutionPolicy>(policy), sets_.begin(), sets_.end(), [&](auto& inner) {
            UniqueLock m(inner);
            std::for_each(inner.set_.begin(), inner.set_.end(), fCallback);
          });
      }
      ...
      size_type erase(const key_arg<K>& key)
      {
        ...
        return erase_if_impl<K, decltype(always_erase), ReadWriteLock>(key,
                                                                       std::move(always_erase));
      }
    ...
    private:
      ...
      template<class Mtx2_>
      parallel_hash_set&
      move_assign(parallel_hash_set<N, RefSet, Mtx2_, Policy, Hash, Eq, Alloc>&& that,
                  std::true_type)
      {
    ...
    class parallel_hash_map : public parallel_hash_set<N, RefSet, Mtx_, Policy, Hash, Eq, Alloc>
    {
      ...
      template<class P>
      using MappedReference =
        decltype(P::value(std::addressof(std::declval<typename parallel_hash_map::reference>())));

      // MappedConstReference<> may be a non-reference type.
      template<class P>
      using MappedConstReference = decltype(
        P::value(std::addressof(std::declval<typename parallel_hash_map::const_reference>())));
    ...
    public:
      ...
      template<class K = key_type, class... Args>
      iterator
      try_emplace_with_hash(size_t hashval, const_iterator, const key_arg<K>& k, Args&&... args)
      {
    ...
    private:
      ...
      std::pair<iterator, bool> try_emplace_impl(K&& k, Args&&... args)
      {
        return try_emplace_impl_with_hash(
          this->hash(k), std::forward<K>(k), std::forward<Args>(args)...);
      }
    ...
    auto DecomposePair(F&& f, Args&&... args)
      -> decltype(memory_internal::DecomposePairImpl(std::forward<F>(f),
  ...
  class flat_hash_set
    : public phmap::priv::raw_hash_set<phmap::priv::FlatHashSetPolicy<T>, Hash, Eq, Alloc>
  {
  ...
  class flat_hash_map
    : public phmap::priv::raw_hash_map<phmap::priv::FlatHashMapPolicy<K, V>, Hash, Eq, Alloc>
  {
  ...
  template<class T, class Hash, class Eq, class Alloc> // default values in phmap_fwd_decl.h
  class node_hash_set
    : public phmap::priv::raw_hash_set<phmap::priv::NodeHashSetPolicy<T>, Hash, Eq, Alloc>
  {
  ...
  class node_hash_map
    : public phmap::priv::raw_hash_map<phmap::priv::NodeHashMapPolicy<Key, Value>, Hash, Eq, Alloc>
  {
  ...
  class parallel_flat_hash_set
    : public phmap::priv::parallel_hash_set<N,
  ...
  class parallel_flat_hash_map
    : public phmap::priv::parallel_hash_map<N,
                                            phmap::priv::raw_hash_set,
                                            Mtx_,

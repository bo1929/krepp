...
Record::Record(record_sptr_t source1, record_sptr_t source2)
{
  ...
  node_sptr_t root =
    Tree::compute_lca(source1->get_tree()->get_root(), source2->get_root()->get_root());
  tree = root->get_tree();
}
...
inline se_t Record::map_compact(sh_t sh) { return sh_to_se[sh]; }

inline record_sptr_t Record::getptr() { return shared_from_this(); }

inline tree_sptr_t Record::get_tree() { return tree; }

inline bool CRecord::check_node(se_t se) { return se_to_node.contains(se); }

inline node_sptr_t CRecord::get_node(se_t se) { return se_to_node[se]; }

inline crecord_sptr_t CRecord::getptr() { return shared_from_this(); }

inline se_t CRecord::get_nnodes() { return se_to_node.size(); }

inline pse_t CRecord::get_pse(se_t se) { return se_to_pse[se]; }
